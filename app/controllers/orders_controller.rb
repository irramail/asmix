      ...
      order.plists.each do |file|
        if file.mediafile.present?
          if order.device.tasks.where(mediafile_id: file.mediafile.id).empty?
            order.tasks.create(device_id: order.device.id, typeoftask_id: 1, typeofstatus_id: 1, options: "<URLS><URL>#{file.mediafile.file}|#{file.mediafile.md5[-4..-1]}</URL></URLS>", mediafile_id: file.mediafile.id)

        ...
        @plsbgmusic.devices.each do |device|
          ...
          @plsbgmusic.mediafiles.each do |mediafile|
            if device.tasks.where(mediafile_id: mediafile.id).empty?
              device.tasks.create(typeoftask_id: 1, typeofstatus_id: 1, options: "<URLS><URL>#{mediafile.file}|#{mediafile.md5[-4..-1]}</URL><NAME>#{mediafile.filename}</NAME></URLS>", mediafile_id: mediafile.id)
